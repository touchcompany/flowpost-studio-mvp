  ...
  return {
    ...
    async getSession(profileId = "demo-profile") {
      ...
      return {
        ...
        moduleAccessMode: profile.metadata?.moduleAccessMode || "auto",
        enabledModules: Array.isArray(profile.metadata?.enabledModules) ? profile.metadata.enabledModules : [],
    ...
    async listProfiles() {
      ...
      try {
        profiles = await request("app_profiles", { query: "select=id,name,email,provider,role,role_label,company_access,metadata,created_at,updated_at&deleted_at=is.null&order=updated_at.desc" });
      } catch (error) {
        if (!isMissingSoftDeleteColumn(error)) throw error;
        profiles = await request("app_profiles", { query: "select=id,name,email,provider,role,role_label,company_access,metadata,created_at,updated_at&order=updated_at.desc" });
      }
