...
function createSupabaseStore() {
  ...
  async function optionalRequest(table, options = {}) {
    try {
      ...
      if (String(error.message || "").includes(`Supabase ${table} 404`)) return [];
      if (String(error.message || "").includes("PGRST125")) return [];
      throw error;
    }
  }
  ...
  return {
    ...
    async getState() {
      const [companiesRows, sourcesRows, accountRows, assetRows, postRows, jobRows, memberRows, promptRows, inviteRows, appRecordRows] = await Promise.all([
        ...
        request("prompt_templates", { query: "select=*" }),
        request("company_invitations", { query: "select=*" }),
    ...
    async saveState(state) {
      ...
      await clearCompanyScoped("prompt_templates", companyIds);
      await clearCompanyScoped("company_invitations", companyIds);
      try {
        await clearRecords(["agency", "client", "invoice", "agency_service", "service_order", "activity", "billing_draft"]);
      } catch (error) {
    ...
    async getSession(profileId = "demo-profile") {
      ...
      const profile = await getFirst("app_profiles", `select=*&id=eq.${safeProfileId}&deleted_at=is.null&limit=1`);
    ...
    async listDeletedProfiles() {
      return request("app_profiles", { query: "select=id,name,email,provider,deleted_at,deletion_expires_at&deleted_at=not.is.null&order=deleted_at.desc" });
    },

    async listProfiles() {
      const [profiles, subscriptions] = await Promise.all([
        request("app_profiles", { query: "select=id,name,email,provider,role,role_label,company_access,created_at,updated_at&deleted_at=is.null&order=updated_at.desc" }),
        request("subscriptions", { query: "select=profile_id,plan,plan_label,status" }),
      ]);
