...
function withoutCompanyRecords(db, companyIds) {
  const ids = new Set(companyIds);
  const keep = (items = []) => items.filter((item) => !ids.has(item.companyId));
  return {
    ...db,
    companies: (db.companies || []).filter((company) => !ids.has(company.id)),
    publications: keep(db.publications),
    jobs: keep(db.jobs),
    clients: keep(db.clients),
    accessMembers: keep(db.accessMembers),
    accessInvites: keep(db.accessInvites),
    invoices: keep(db.invoices),
    serviceOrders: keep(db.serviceOrders),
    activityLog: keep(db.activityLog),
    financeTransactions: keep(db.financeTransactions),
    monthlyProviders: keep(db.monthlyProviders),
    promptLibrary: keep(db.promptLibrary),
  };
}
...
function createLocalStore() {
  return {
    ...
    async getSession(profileId = "") {
      const db = readDb();
      if (profileId && db.sessions?.[profileId]) return db.sessions[profileId];
    ...
    async getProfileByEmail(email = "") {
      ...
      return (
        Object.values(readDb().sessions || {}).find((session) => String(session.email || "").trim().toLowerCase() === normalizedEmail && !isDeleted(session)) ||
        null
      );
    },
    ...
    async getProfileByPasswordResetToken(tokenHash = "") {
      ...
      return (
        Object.values(readDb().sessions || {}).find((session) => session.metadata?.passwordReset?.tokenHash === tokenHash && !isDeleted(session)) || null
      );
    },
    ...
    async saveSession(session) {
      ...
      const sessions = {
        ...(db.sessions || {}),
        [nextSession.id || "demo-profile"]: nextSession,
      };
      writeDb({ ...db, session: nextSession, sessions });
      return nextSession;
    },
    ...
    async deleteProfile(id, deletedBy = "system") {
      ...
      const sessions = Object.fromEntries(
        Object.entries(db.sessions || {}).map(([key, session]) => [
          key,
          session.id === id ? { ...session, ...deletionWindow(), deletedBy } : session,
        ])
      );
      writeDb({ ...db, sessions });
    ...
    async permanentlyDeleteCompany(id) {
      const db = readDb();
      const nextDb = withoutCompanyRecords(db, [id]);
      const activeCompanyId = nextDb.activeCompanyId === id ? nextDb.companies.find((company) => !isDeleted(company))?.id || "" : nextDb.activeCompanyId;
      writeDb({ ...nextDb, activeCompanyId });
      return { permanentlyDeleted: true, id, activeCompanyId };
    },
    ...
    async purgeExpiredDeletedCompanies() {
      ...
      writeDb(withoutCompanyRecords(db, expiredIds));
      return { purged: expiredIds.length, ids: expiredIds };
    },

    async listDeletedProfiles() {
      return Object.values(readDb().sessions || {}).filter(isDeleted);
    },
    ...
    async restoreProfile(id) {
      ...
      const sessions = Object.fromEntries(
        Object.entries(db.sessions || {}).map(([key, session]) => [
          key,
          session.id === id ? { ...session, deletedAt: "", deletionExpiresAt: "", deletedBy: "" } : session,
        ])
      );
      writeDb({ ...db, sessions });
